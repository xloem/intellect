...
sed '/^\s\s*METHOD.*)\s*{.*}\s*$/ { s/^\s\s*METHOD\s\s*\(\S*\)\s\s*\([^(]*\)\s*(\([^)]*\))\s*{\(.*\)}\s*$/static reference \2(){\/\*METHOD\*\/static reference storage({string("\2"),(function<as_reference<\1>( \3)>)\n[]( \3) -> as_reference<\1> { using __return-type = \1; \4 }); return storage;}/; s/\sreturn /return (reference)(__return-type)/; }' |
# multi-line in-header method definitions
sed '/^\s\{0,1\}METHOD/,/^\s\{0,1\}}/ { /^\s*METHOD.*)\s*$/N; s/METHOD\s\s*\(\S*\)\s\s*\([^(]*\)\s*(\([^)]*\))\s*{/\/\*METHOD\*\/static reference \2(){static reference storage({string("\2"),(function<as_reference<\1>( \3)>)\n[]( \3) -> as_reference<\1> { using __return-type = \1;/; s/return /return (reference)(__return-type)/; s/^\s}/&); return storage;}/ }' |
# out-of-header method definitions
sed '/^METHOD/,/^}/ { /^\s*METHOD.*)\s*$/N; s/METHOD\s\s*\(\S*\)\s\s*\([^(]*\)\s*(\([^)]*\))\s*/\/\*METHOD\*\/reference \2(){static reference storage({string("\2"),(function<as_reference<\1>( \3)>)\n[]( \3) -> as_reference<\1> { using __return-type = \1;/; s/return /return (reference)(__return-type)/; s/^}/&); return storage;}/ }' |
...
sed ':loop s/\(\*METHOD\*.*function<as_reference<[^>]*>([^)]* \)\(\S*[^\/,)]\)\( [^ ,)]*\)\([,)]\)/\1reference\/\*\*\/\4/g;t loop' |
