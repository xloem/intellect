...
char const * dump_ex(ref what, unsigned depth, char const * indentation, bool reset)
{
	...
	static thread_local std::unordered_map<ref, std::pair<std::string, unsigned>> found;
	if (reset) { found.clear();}
	std::string result;
	if (found.count(what)) {
		result = found[what].first;
		if (found[what].second >= depth) {
			depth = 0;
		}
	...
	if (!result.size()) {
		...
		} else if (what.count(sym::is, sym::habit)) {
			result = "<habit>";
		}
		...
		found[what] = std::pair(result, depth);
	}
	if (depth) {
		result = "[" + result + ":\n";
		bool first = true;
