...
public:
	...
	template <class... input_ts>
	cxxhabit(std::string name, il<text> outputs, void(*function)(input_ts...), il<text> inputs)
	: ref({
		{sym::is, sym::habit},
		{sym::inputs, seq(*(il<ref>*)&inputs)},
		{sym::outputs, seq(*(il<ref>*)&outputs)},
		{sym::habit, ref({},reinterpret_cast<void(*)(decltype(((input_ts*)0)->template as<ref>())...)>(function))}
	}, name)
	{ }

	template <class outputs_t, class... input_ts>
	cxxhabit(std::string name, il<text> outputs, std::vector<outputs_t>(*function)(input_ts...), il<text> inputs)
	: ref({
		{sym::is, sym::habit},
		{sym::inputs, seq(*(il<ref>*)&inputs)},
		{sym::outputs, seq(*(il<ref>*)&outputs)},
		{sym::habit, ref({},reinterpret_cast<std::vector<ref>(*)(decltype(((input_ts*)0)->template as<ref>())...)>(function))}
	}, name)
	{ }
	...
	: ref({
		...
		{sym::inputs, seq(*(il<ref>*)&inputs)},
		{sym::outputs, seq(*(il<ref>*)&outputs)},
		{sym::habit, ref({},reinterpret_cast<ref(*)(decltype(((input_ts*)0)->template as<ref>())...)>(function))}
	}, name)
	{ }
	...
	inline void call_with_ctx(ref context)
	{
		...
		if (cxx_takes_context()) {
			ctxdata()(context);
		} else {
			seq outputs_seq = self[sym::outputs].as<seq>();
			std::vector<ref> outputs_vec;

			switch (outputs_seq.size()) {
			case 0:
				call_cxx_function<void>(context);
				break;
			case 1:
				outputs_vec.push_back(call_cxx_function<ref>(context));
				break;
			default:
				outputs_vec = call_cxx_function<std::vector<ref>>(context);
			}

			unsigned index = 0;
			for (ref outputname : outputs_seq) {
				context.set(outputname, outputs_vec[index]);
				++ index;
			}
...
private:
	...
	return_type call_cxx_function(ref inputs_ctx)
	{
		seq inputs_seq = self[sym::inputs].as<seq>();

		std::vector<ref> args(inputs_seq.size());
		unsigned count = 0;
		for (ref value : inputs_seq) {
			args[count] = inputs_ctx[value];
			++ count;
		}

		ref function = get(sym::habit);
		switch (args.size()) {
		case 0:
			return function.data<return_type(*)()>()();
		case 1:
			return function.data<return_type(*)(ref)>()(args[0]);
		case 2:
			return function.data<return_type(*)(ref,ref)>()(args[0],args[1]);
		case 3:
			return function.data<return_type(*)(ref,ref,ref)>()(args[0],args[1],args[2]);
		case 4:
			return function.data<return_type(*)(ref,ref,ref,ref)>()(args[0],args[1],args[2],args[3]);
		case 5:
			return function.data<return_type(*)(ref,ref,ref,ref,ref)>()(args[0],args[1],args[2],args[3],args[4]);
		default:
			throw exception({});
		}
	}
