...
int main()
{
	...
	if (testreadwritefunc(designedpassinghabit)) {
		*output_stream << "designed passing habit passed" <<std::endl;
	} else {
		...
		*output_stream << "designed passing habit failed" <<std::endl;
		return 0;
	}
	...
	while (true) {
		...
		*output_stream << dump(habit) << std::endl;
