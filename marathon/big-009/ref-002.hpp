...
// this can be made a node using virtual ref methods, if helpful
...
struct con {
	...
	// a side effect is that equal-typed links keep their insertion order (i may have run into an issue using that for ordered lists, unsure: it might be more general to pair ordering)
