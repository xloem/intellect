...
std::vector<ref> forever_seq_gen_setup(gen_use gen);
void forever_seq_gen_next(ref ctx);
gen forever_seq_gen(
	{"forever_seq_gen_setup",{sym::gen_use, sym::work},forever_seq_gen_setup, {sym::gen_use}},
	{{sym::seq},{sym::gen_use, sym::work},forever_seq_gen_next});
std::vector<ref> forever_seq_gen_setup(gen_use gen)
{
	//gen_use gen = ctx[sym::gen_use].as<gen_use>();
	return {gen, // gen_use
		seq({ref({{sym::work,seq({})}})}) // work
	};
	/*
		{sym::gen_use, gen},
		{sym::work, seq({
...
// draft of a similarity-explorer
struct integer : public ref
{
...
void process(seq refs)
{
	// setup
	std::unordered_map<unsigned long, std::unordered_multimap<ref,ref>> found;
	seq item_queue;
	for (ref item : refs) {
		item_queue.push_back(ref({
			{sym::what, item},
			{sym::to, item},
			{sym::hash, integer(std::hash<ref>()(item))},
			{sym::depth, integer(0)}
		}));
	}
	...
	// next depth
	seq next_queue;
	for (ref item : item_queue) {
		ref what = item[sym::what];
		ref to = item[sym::to];
		unsigned long hash = item[sym::hash].as<integer>().data();
		unsigned long depth = item[sym::depth].as<integer>().data();

		// - get all links from to, making new 'to's
		for (auto link : to->refs) {

			// WIP <-

			(void)hash;
			(void)depth;
			// please do simpler work until you can make more than 1 line of progress without spasming
			// 	first choice is to copy this function and the one in gens.hpp
			// 	simply copy them.
		}
	}
}
...
cxxhabit write("write", {},{sym::text},write_func);
void write_func(ref context)
{
	*output_stream << context[sym::text].data<std::string>();
}
...
text read_func();
cxxhabit read("read", {variable(sym::text)},read_func,{});
text read_func()
{
	std::string word;
	before_read_handler();
...
cxxhabit writeseq("writeseq", {},{sym::seq, sym::text, sym::next},writeseq_func);
void writeseq_func(ref ctx)
{
	...
	for (auto item : list) {
		if (first) {
			...
			*output_stream << ctx[sym::text].as<text>().get();
		}
		*output_stream << item.as<text>().get();
	}
	*output_stream << ctx[sym::next].as<text>().get();
};
//stephabit printlist({},{variable(sym::seq)},{
//	{{},{sym::seq, txt::comma, txt::null}, writeseq}
//});
void printlist_func(seq list);//(ref ctx);
//cxxhabit printlist("printlist", {},{sym::seq},printlist_func);
cxxhabit printlist("printlist", {}, printlist_func, {sym::seq});
void printlist_func(seq list)//(ref ctx)
{
	//seq list = ctx[sym::seq].as<seq>();
	bool first = true;
...
cxxhabit printsteps("printsteps", {},{sym::steps},printsteps_func);
void printsteps_func(ref ctx)
{
...
cxxhabit smartwrite("smartwrite", {},{sym::text},smartwrite_func);
void smartwrite_func(ref ctx)
{
...
stephabit smartprompt("smartprompt",{variable(sym::what)},{sym::text},{
	{{},{sym::text},smartwrite},
	{{sym::what},{},read}
});

void setvar_func(ref ctx) {}
cxxhabit setvar("setvar",{sym::what},{sym::what},setvar_func);

void loopbody_func(ref ctx);
cxxhabit loopbody("loopbody",{sym::what},{sym::gen, sym::input},loopbody_func);
void loopbody_func(ref ctx)
{
	gen_use habit_gen = ctx[sym::gen].as<gen_use>();
	stephabit a_habit("generated",{},{},habit_gen.next().as<seq>());
	printsteps({a_habit});
	stephabit readwrite = ctx[sym::input].as<stephabit>();
	text input = readwrite({});
	*output_stream << input.get() << std::endl;
	*output_stream << dump_ex(input, 3, "", true) << std::endl;
	if (input.get() == "q" || !input) {
		ctx.wipe(sym::what);
	} else {
...
int main()
{
	...
	//}
	stephabit promptword("promptword",{word},{},{
		{{},{text("enter a word:\n")}, write},
		{{word},{}, read}
	});
	stephabit showword("showword",{},{word},{
	...
	stephabit helloworld("helloworld",{},{},{
		{{},{text("hello")}, write},
		{{},{txt::space}, write},
		{{},{text("world")}, write},
		{{},{txt::endl}, write}
	...
	stephabit readwrite("readwrite",{word},{},{
	...
	stephabit designedpassinghabit("readwriteexample",{},{},{
	...
	while (true) {
		...
		stephabit habit("generatedhelloworld",{},{},steps);
		//printsteps(habit);
	...
	while (true) {
		++ count;
		seq steps = habit_gen.next().as<seq>();
		stephabit habit("generatedreadwrite",{},{},steps);
		//printsteps(habit);
