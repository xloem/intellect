...
{
	return create({
		{symbols::outputs, seq(outputs)},
		{symbols::inputs, seq(inputs)}
	}, code);
}

// create a step in a runtime habit
ref create(il<ref> outputs, il<ref> inputs, ref what)
{
	return create({
		{symbols::outputs, seq(outputs)},
		{symbols::inputs, seq(inputs)},
...
ref start_run = habit({symbols::state}, {symbols::steps}, [](ref context){
		// minor problem: steps must override context passed to habit
		// simplest solution is to use unique symbol
...
// run a single step in a context that has been set up
ref step_run = habit({symbols::outputs}, {symbols::state}, [](ref outer_context) {
	...
	ref subcontext = create({{symbols::outers, context}});
