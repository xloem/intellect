...
ref step_run = habit({symbols::outputs}, {symbols::state}, [](ref outer_context){
	...
	for (ref input1 = inputs->get(symbols::first), input2 = action->get(symbols::inputs)->get(symbols::first); input1->isthing() && input2->isthing(); input1 = input1->get(symbols::next), input2 = input2->get(symbols::next)) {
		ref value = input1;
	...
	for (ref output1 = outputs->get(symbols::first), output2 = action->get(symbols::outputs)->get(symbols::first); output2->isthing() && output1->isthing(); output1 = output1->get(symbols::next), output2 = output2->get(symbols::next)) {
		ref value = output2;
		if (value->get(symbols::variable)->isthing()) {
