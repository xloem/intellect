	...
	void call_with_ctx(ref context)
	{
		...
		for (ref inputname : self[sym::inputs].as<seq>()) {
			...
			inputs.push_back(ref({
				{sym::what, input},
				{sym::from, input.back(sym::from)} // TODO BUG: incorrect if multi-threaded, could use flow id (or thread id) to fix
			}));
		}
		ref step_ctx = context[sym::step_context];
		if (step_ctx) { step_ctx = step_ctx.clone(); }
		ref call{{
			{sym::inputs, inputs},
			{sym::outputs, outputs},
			{sym::step_context, step_ctx},
		}};
...
struct stephabit : public cxxhabit
{
	...
	: cxxhabit(outputs, inputs, stephabit::run)
	{
		set(sym::what, steps);
