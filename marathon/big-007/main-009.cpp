...
int main()
{
	...
	cxxhabit printsteps({},{sym::steps},[&](ref ctx) // & because uses printlist
	{
	...
	cxxhabit loopbody({sym::what},{sym::gen},[&](ref ctx) // & because uses habit_gen
	{
	...
	stephabit mainloop({},{},{
		...
		{{var(sym::what)},{var(sym::what),seq({ // todo? change whilesteps to not rely on outer context, or to copy outer context in fully
			step({sym::what},{habit_gen},loopbody),
			step({},{sym::what},write)
