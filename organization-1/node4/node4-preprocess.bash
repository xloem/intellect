...
sed -f - "$1" >> "$2" <<-"END"
	...
	:loop s/\(\*``METHOD``\*.*std::function<reference\/\*``[^\*]*``\*\/([^)]* \)\(\S*[^\/,)]\)\( [^ ,)]*\)\([,)]\)/\1reference\/\*````\*\/\4/g;t loop
